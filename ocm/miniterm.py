...
# This version was adapted to defaut to one-net communication
# by Paolo Patruno <p.patruno@iperbole.bologna.it>
# implemented device locking and good default
...
class Miniterm:
    def __init__(self, port, baudrate, parity, rtscts, xonxoff, echo=False, convert_outgoing=CONVERT_LF, repr_mode=0):

        if onenet.lockdev(port) <> 0 :
            raise Exception("cannot lock device: "+repr(port))
            ...
            while self.alive:
                ...
                if menu_active:
                    ...
                    if self.echo:
                        if self.repr_mode == 3:
                            # escape everything (hexdump)
                            sys.stdout.write("<-%s " % self.newline.encode('hex'))
                        else:
...
def main():
    ...
    parser.add_option("--crlf",
        dest = "crlf",
        action = "store_true",
        help = "do not send LF, send CR+LF",
        default = False
    )
...
3: hex dump everything""",
    ...
    if options.cr and options.crlf:
        parser.error("only one of --cr or --crlf can be specified")
